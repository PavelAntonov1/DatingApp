...
const corsOptions = {
  origin: "https://66b94aeca80f80ced7489f64--keen-conkies-607c6f.netlify.app",
  ...
  try {
    ...
    if (isMatch) {
      ...
      res.setHeader(
        ...
        "https://66b94aeca80f80ced7489f64--keen-conkies-607c6f.netlify.app"
      );
      res.setHeader("Access-Control-Allow-Credentials", true);
...
app.post("/api/send-verification-email", (req, res) => {
  ...
  const mailOptions = {
    from: MAILRU_EMAIL,
    to: userEmail,
    subject: "Your confirmation code for registration for Dating App",
